...
在「样式」设置中开启「悬浮播放器」并填写歌单 ID 后，播放器会固定显示在前台左下角。默认接口使用网易云音乐的 Meting 歌单格式，可在后台选择顺序或随机作为默认播放模式，并会尝试自动播放；若浏览器拦截自动播放，会在用户首次点击页面后继续播放。
